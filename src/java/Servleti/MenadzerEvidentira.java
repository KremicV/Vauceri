           ...
           String br = request.getParameter("brojRestorana");
            ...
            if(brojVaucera!=null && brojVaucera.length()>0){
                try{
                     ...
                     evidentiraj.execute();
                     out.println("<h2 id=\"uspesno\">Vaučer je uspešno evidentiran kao iskorišćen</h2>");
                     request.getRequestDispatcher("Menadzer.jsp").include(request, response);
                        } else{
                            out.println("<h2 id=\"greska\">Ne možete promeniti status vaučera jer vaučer istekao</h2>");
                        request.getRequestDispatcher("Menadzer.jsp").include(request, response);
                        }} else if(status.equals("Ponisten") && brojRestorana.equals(br)){
                        out.println("<h2 id=\"greska\">Ne možete promeniti status vaučera jer vaučer nije aktivan!</h2>");
                        request.getRequestDispatcher("Menadzer.jsp").include(request, response);
                     } else if(status.equals("Iskoriscen") && brojRestorana.equals(br)){
                        out.println("<h2 id=\"greska\">Ne možete promeniti status vaučera jer vaučer nije aktivan!</h2>");
                        request.getRequestDispatcher("Menadzer.jsp").include(request, response);
                     }} else {
                        out.println("<h2 id=\"greska\">Ne možete promeniti status vaučera jer ne pripada Vašem restoranu!</h2>" );
                        request.getRequestDispatcher("Menadzer.jsp").include(request, response);
                     }
                ...
                out.println("<h2 id=\"greska\">Morate upisati broj vaučera</h2>");
                request.getRequestDispatcher("Menadzer.jsp").include(request, response);
            }
